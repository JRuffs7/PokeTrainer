...
class TradeView(discord.ui.View):
	...
	async def cancel_button(self, inter: discord.Interaction):
		if inter.user.id == self.targettrainer.UserId:
			await self.message.edit(content=f'<@{self.trainer.UserId}>\nTrade offer was rejected.', embed=None, view=None)
		else:
	...
	async def submit_button(self, inter: discord.Interaction):
		if self.initial:
			...
			if not self.targettrainer or not self.targetpkmnchoice:
				commandlockservice.DeleteLock(self.trainer.ServerId, self.trainer.UserId)
				self.stop()
				return await self.message.edit(content=f'The target trainer no longer owns the desired Pokemon. Try again.', embed=None, view=None)
			...
			embed = discordservice.CreateEmbed(
				...
				TradeColor,
				thumbnail=botImage)
			await inter.followup.send(content=f'<@{self.targettrainer.UserId}>', embed=embed, view=self, ephemeral=False)
			...
			commandlockservice.DeleteLock(self.trainer.ServerId, self.trainer.UserId)
			self.stop()
			return await self.message.edit(content=f'<@{self.trainer.UserId}> traded away **{pokemonservice.GetPokemonDisplayName(self.userpkmnchoice, self.userdata)}** to <@{self.targettrainer.UserId}> for **{pokemonservice.GetPokemonDisplayName(self.targetpkmnchoice, self.targetdata)}**', embed=None, view=None)
