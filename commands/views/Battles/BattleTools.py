...
class PokemonSelector(discord.ui.Select):
	def __init__(self, team: list[Pokemon], data: list[PokemonData], active: Pokemon):
			options=[discord.SelectOption(
						label=f'{pokemonservice.GetPokemonDisplayName(t, next(p for p in data if p.Id == t.Pokemon_Id))}',
            description= f'{pokemonservice.GetOwnedPokemonDescription(t, next(p for p in data if p.Id == t.Pokemon_Id))}',
						value=t.Id
					) for t in team if t.Id != active.Id]
