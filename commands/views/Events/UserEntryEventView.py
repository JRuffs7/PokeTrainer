...
class UserEntryEventView(EventView):
	...
	async def drop_button(self, interaction: discord.Interaction, button: discord.ui.Button):
		...
		return await interaction.response.send_message(content=f"Successfully dropped out of the contest.", ephemeral=True)
	...
	async def enter_button(self, interaction: discord.Interaction, button: discord.ui.Button):
		if str(interaction.user.id) not in self.server.CurrentEvent.EventEntries:
			...
			if self.server.CurrentEvent.EventType == EventType.StatCompare.value:
				...
				return await interaction.response.send_message(content=f"You have entered the event with {pokemonservice.GetPokemonDisplayName(entry)} with a {f'height of {entry.Height}' if heightBased else f'weight of {entry.Weight}'}.", ephemeral=True)
			else:
				self.server.CurrentEvent.EventEntries[str(interaction.user.id)] = entry
				await self.SendToThread(interaction, True)
				return await interaction.response.send_message(content=f"You have entered the event with a count of {entry}.", ephemeral=True)
		return await interaction.response.defer()
